	...
	return (
		<>
			<div className="rounded p-3 d-flex flex-column my-5 w-100 border border-dark" style={{ boxShadow: "10px 10px 0px 0px rgba(0,0,0,1)" }}>
				...
				<ul>
					<li>Name: {props.order.shipping.name}</li>
					<li>Email: {props.order.shipping.email}</li>
					<li>Phone: {props.order.shipping.phone}</li>
					<li>First Address: {props.order.shipping.address.line1}</li>
					<li>Second Address: {props.order.shipping.address.line2}</li>
				...
				<ul>
					{
						...
						(
							<li key={uuidv4()}>{product.description} ($ {(product.amount_total / 100) / product.quantity}) - {product.quantity > 1 ? (<>{product.quantity} items</>) : (<>{product.quantity} item</>)} ordered</li>
						)
						)
					}
				</ul> <hr />
